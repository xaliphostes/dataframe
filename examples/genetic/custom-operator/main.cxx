...
int main() {
    ...
    std::vector<Item> items = {{1, "Laptop", 3.0, 1500},
                               {2, "Smartphone", 0.3, 1000},
                               {3, "Tablet", 1.0, 800},
                               {4, "Camera", 0.5, 700},
                               {5, "Headphones", 0.2, 300},
                               {6, "External HDD", 0.7, 200},
                               {7, "Power Bank", 0.4, 150},
                               {8, "Speaker", 1.5, 400},
                               {9, "E-reader", 0.3, 250},
                               {10, "Smartwatch", 0.1, 350},
                               {11, "Portable Monitor", 2.0, 600},
                               {12, "Gaming Console", 2.5, 450},
                               {13, "Wireless Earbuds", 0.1, 200},
                               {14, "Bluetooth Mouse", 0.2, 80},
                               {15, "USB Hub", 0.3, 100}};
    ...
    auto fitness_function = [&items,
                             weight_limit](const df::Serie<int> &solution) {
    ...
    // ratio items
    auto custom_crossover = [&items](const df::Serie<int> &parent1,
                                     const df::Serie<int> &parent2) {
        ...
        return std::make_pair(df::Serie<int>(child1_values),
                              df::Serie<int>(child2_values));
    };
    ...
    auto custom_mutation = [&items](const df::Serie<int> &individual) {
        ...
        for (size_t i = 0; i < mutated_values.size(); ++i) {
            double mutation_prob =
                0.1 * std::exp(-10 * std::pow(ratios[i] - 0.5, 2));

            if (dis(gen) < mutation_prob) {
                mutated_values[i] =
                    1 - mutated_values[i]; // Flip 0 to 1 or 1 to 0
            }
        }
    ...
    std::cout << "Solving Knapsack Problem with " << items.size() << " items"
              << std::endl;
    std::cout << "Weight limit: " << weight_limit << " kg" << std::endl;
    std::cout << "------------------------------------------" << std::endl;

    // Run the optimization (maximize total value)
    auto [best_solution, best_fitness] = ga.optimize_combinatorial(
    ...
    std::cout << "-------------------" << std::endl;
    std::cout << std::left << std::setw(20) << "Item" << std::setw(10)
              << "Weight" << std::setw(10) << "Value"
              << "Selected" << std::endl;
    ...
    for (size_t i = 0; i < best_solution.size(); ++i) {
        ...
        std::cout << std::left << std::setw(20) << items[i].name
                  << std::setw(10) << items[i].weight << std::setw(10)
                  << items[i].value << (selected ? "Yes" : "No") << std::endl;
    ...
    std::cout << std::left << std::setw(20) << "Total:" << std::setw(10)
              << total_weight << std::setw(10) << total_value << std::endl;

    std::cout << "\nWeight limit: " << weight_limit << " kg" << std::endl;
    std::cout << "Weight usage: " << (total_weight / weight_limit * 100) << "%"
              << std::endl;
