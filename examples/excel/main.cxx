...
int main() {
    ...
    sales.add("Product", df::Serie<std::string>{"Apple", "Orange", "Banana",
                                                "Apple", "Orange", "Banana"});

    sales.add("Price", df::Serie<double>{0.50, 0.75, 0.30, 0.55, 0.80, 0.35});

    sales.add("Quantity", df::Serie<int>{100, 80, 150, 120, 90, 140});

    sales.add("Date",
              df::Serie<std::string>{"2024-01-01", "2024-01-01", "2024-01-01",
    ...
    auto revenues =
        df::zip(prices, quantities).map([](const auto &tuple, size_t) {
    ...
    auto apple_mask =
        products.map([](const std::string &p, size_t) { return p == "Apple"; });

    // 3. Working with multiple columns (like VLOOKUP or INDEX/MATCH in Excel)
    // Get average price per product
    auto price_by_product =
        df::zip(products, prices).map([](const auto &tuple, size_t) {
    ...
    auto daily_sales =
        df::zip(dates, revenues).map([](const auto &tuple, size_t) {
            auto [date, rev] = tuple;
