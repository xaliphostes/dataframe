...
namespace df {
     ...
     */
    template <typename T>
    Serie<T> rbf_grid_2d(const Grid2D& grid, const Serie<T>& values, const Serie<Vector2>& targets,
        RBFKernel kernel = RBFKernel::Multiquadric, double epsilon = 1.0, uint support_radius = 2);
     ...
     */
    template <typename T>
    Serie<T> rbf_grid_3d(const Grid3D& grid, const Serie<T>& values, const Serie<Vector3>& targets,
        RBFKernel kernel = RBFKernel::Multiquadric, double epsilon = 1.0, uint support_radius = 2);
