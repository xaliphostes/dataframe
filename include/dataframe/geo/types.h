...
namespace df {
    ...
    // C++20 Concepts (cleanest syntax)
    template <typename T>
    concept VectorType = std::same_as<T, Vector2> || std::same_as<T, Vector3>;

    // Vector arithmetic operators
    template <size_t N> Vector<N> operator-(const Vector<N>& a, const Vector<N>& b)
    {
        Vector<N> result;
    ...
    template <size_t N> double length(const Vector<N>& v) { return std::sqrt(dot(v, v)); }
